...
public class FileService {
	...
	public void method2() {
		...
		if(!file.exists()) {
			try {
				// File 클래스의 메서드는 대부분 IOException 발생 시킨
				// -> IOException는 반드시 예외처리르 해야하는 Checked Exception
				// -> 예외처리 필ㅅ
				if(file.createNewFile()) { // 파일 정상 생성된 경우
					System.out.println(file.getName() + "파일 생성 완료");
				}
			} catch (IOException e) {
				e.printStackTrace(); // 예외 정보 + 발생 위치 추적
			}
...
//			System.out.println(lastModified);
			...
			// -> 폴더 크기를 구하는 메서드느 따로 존재X
			// -> 구하려면 폴더 안의 모든 파일을 순회하여 각 파일 크기 합산
