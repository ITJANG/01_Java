...
public class ByteService {
	...
	public void fileByteOutput() { 
		...
		try {
			...
			*/
			...
			// FileOuptputStream에서는 flush() 선택 사항
			...
			// 사용완료한 스트림 제거(닫기, 메모리 반환)하는 코드 작성
			// -> 메모리 누수 방지
			try {
				// 스트림이 정상 생성된 경우
				// 스트림을 닫는다 (NullPointerException 방지)
				// close() 메서드도 IOExceptiond을 발생 시키기에
				// 예외처리 필수
	 ...
	 * -> Map 개열인 Properties 클래스 사용
	 **/
	public void readProperties() {
		
		// Properties 클래스는 Hashtabled의 하위 클래스로 Map 계열 클래스 중 하나
		...
		try {
			...
			System.out.println("user name: " + properties.getProperty("language"));
	 ...
	 * -> BufferedReader는 보토 스트림으로 단독 사용X
	 * -> 생성 시 기반스트림인 InputStream의 보조 스트림인 InputStreamReader 생성
	 * 
	 */
	public void fileCopy() {
		...
		try {
			...
			// target : /io_test/20240926/노래가사_cop.txt
